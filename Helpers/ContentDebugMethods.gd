extends RefCounted
class_name ContentDebug

static func debug_print_site(site: SiteRuntime) -> void:
	print("\n=== SITE ===")
...
static func debug_print_container(container: ContainerRuntime, schema_id: StringName, indent: int) -> void:
	var pad := "  ".repeat(indent)

	print("\n%s[Container] schema_id: %s" % [pad, schema_id])
...
static func debug_print_socket(socket: SocketRuntime, schema_id: StringName, indent: int) -> void:
	var pad := "  ".repeat(indent)

	print("\n%s[Socket] schema_id: %s" % [pad, schema_id])
...
static func debug_print_items(items: Array[ItemRuntime], indent: int) -> void:
	var pad := "  ".repeat(indent)
	for i in items:		
		print("\n%s[Item]" % pad)
